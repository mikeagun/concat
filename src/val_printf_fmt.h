...
typedef struct printf_fmt {
  ...
  enum {
    ...
    PRINTF_F_SPACE   = (1<<3), //( ) leave ' ' before positive number / empty string from signed conversion
  ...
  //Standard conversion specifiers for scanf
  //%          - matches literal %
  ...
  //n          - saves number of chars consumed so far (not counted as a conversion)
